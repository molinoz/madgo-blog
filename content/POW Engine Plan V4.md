...
In a way I am thinking about how advanced minecraft maps get. For example setting up triggers for events to have enemies appear or have tiles change but either way it is too much to think about right now.
## Clubs (Social)
A pretty experimental idea, its basically a feature to start groups people can join. Which is pretty simple on its own since we don't have to handle their communication because they can just create a discord server for their club. There are certain benefits like sharing paid content maybe(Like a real life club where you would borrow someone's rulebook).  

The experimental part would be being able to location lock your club for example to only have members from your country, state, county, city. Or to an organization like a school/college/company. Don't know how verification would be handled but its just a cool thought
...
Having actions set off visual effects could make things interesting Foundry is able to do that but it is also out of the budget.
...
pretty much what it says. The point is to easy the communication between people who use text chat and only voice chat. It works well for accessibility. 

Another purpose for it is to create logs in screen play format that shows what players have said in character text or voice and their characters actions.
## Random Content Generator (Engine)
Used to generate npcs and monsters. The most useful direction is creating a framework for people to create their own generators.
## The use of LMMs (Engine)
Large Language Models possibly to make conversational NPCs but who knows how useful that will especially when running things locally and how smart the LMMs can seem to be.
## 3d Board(Engine)
At first this was way up on the list but a lot of functionality can be done with 2d and thinking about keeping the same level of features for was a head ache. So I will not unless there interest from customers for
# Time line
## Phase 1: ***POW: System Builder*** _demo_
The ***POW: System Builder demo*** is for only a single user with the *Object Editor*, *Sheet Editor*, *Wiki-like Notes* and Basic *Rules Referee*. It is to get user feedback on the interfaces to create a system. Along with getting as many people to test it out as possible by making it free.
